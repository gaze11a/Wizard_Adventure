...
// 未実装機能
// 
// 敵のスポーンを徐々に早める、ドラゴンの追加
// ライフ機能（最初3つ、火に当たったら1個マイナス、ドラゴンは即死）
// 効果音の追加
// 設定（効果音音量調節、
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        for (int j = 0; j < MAX_BULLETS; j++) {
            if (game.bullets[j].active && CheckCollision(game.bullets[j], enemy)) {
                ...
                enemy.x = prevX + GetRand(maxGap - minGap) + minGap; // 直前の敵との間隔を確保
                enemy.y = 120 + i * 100;
                enemy.spawn = GetRand(300) + 600; // 倒された後の再出現もさらに遅めに
                game.bullets[j].active = false;
                game.score += 100; // 敵を倒すとポイント加算
            }
        }
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    while (ScreenFlip() == 0 && ProcessMessage() == 0 && ClearDrawScreen() == 0 && gpUpdateKey() == 0) {
        ...
        if (game.state == PAUSED) {
            ...
            if ((game.L - game.shot) == 5000 && game.bulletnum < 5) game.bulletnum++; // 弾を撃ってから5000mごとに弾追加
